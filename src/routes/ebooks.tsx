...
export const Route = createFileRoute("/ebooks")({
  ...
  head: () => ({
    meta: [
      { title: "E-books & Resources — JPCann Associates Limited" },
      {
        name: "description",
        content:
          "Download expert business guides, whitepapers, and e-books from JPCann Associates.",
      },
    ],
  }),
});
...
function useEbooks() {
  const [data, setData] = useState<EbookRow[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (cancelled) return;
      if (!error && rows) setData(rows as EbookRow[]);
      setIsLoading(false);
    })();
    return () => {
      cancelled = true;
...
function EbooksPage() {
  ...
  const [selectedEbook, setSelectedEbook] = useState<EbookRow | null>(null);
  ...
  return (
    <SiteLayout>
      <PageHero
        eyebrow="Resources"
        title="Expert insights, delivered to your inbox."
        description="Browse our library of professional e-books and industry whitepapers. Download your free copy today."
      />
              ...
              {paginated.map((b, idx) => (
                ...
                >
                  ...
                  <div className="flex flex-1 flex-col p-6">
                    ...
                    <button
                      type="button"
                      onClick={() => setSelectedEbook(b)}
                      className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-md bg-primary py-3 text-sm font-semibold text-primary-foreground shadow-sm transition-all hover:bg-primary/90"
                    >
