...
export const Route = createFileRoute("/ebooks/$slug")({
  ...
  head: ({ loaderData }) => {
    const t = loaderData;
    if (!t) return { meta: [{ title: "E-book — JPCann Associates" }] };
    return {
      meta: [
        { title: `${t.title} — JPCann Associates` },
        {
          name: "description",
          content:
            t.description ?? `Download ${t.title} from JPCann Associates.`,
        },
        { property: "og:title", content: `${t.title} — JPCann Associates` },
        {
          property: "og:description",
          content: t.description ?? `Download ${t.title}.`,
        },
        ...(t.image_url
          ? [
    ...
    <SiteLayout>
      <div className="mx-auto max-w-3xl px-4 py-32 text-center md:px-6">
        ...
        <p className="mt-3 text-muted-foreground">
          The e-book you are looking for is no longer available or the link is
          invalid.
        </p>
        <Link
          to="/ebooks"
        ...
        >
          <ArrowLeft size={16} /> Back to E-books
        </Link>
      </div>
    </SiteLayout>
  ),
    ...
    <SiteLayout>
      ...
      <section className="mx-auto max-w-7xl px-4 py-12 md:grid md:grid-cols-3 md:gap-10 md:px-6 md:py-16">
        ...
        <aside className="mt-8 md:mt-0">
          ...
          >
            {/* E-book Cover Image in sidebar (full and uncropped) */}
            {t.image_url && (
            ...
            >
              <Download size={15} /> Download E-Book
            </button>
          </Reveal>
        </aside>
      </section>
