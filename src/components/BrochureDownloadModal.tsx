    ...
    try {
      ...
      supabase
        ...
        .insert({
          ...
          email: values.email,
          subject: `Brochure Download: ${brochure?.title}`,
          message: `User requested download for brochure: ${brochure?.title} (ID: ${brochure?.id}). Phone: ${values.phone}`,
        })
        .then(({ error }) => {
      ...
      if (brochure?.metadata?.file_path) {
        const { data } = supabase.storage
          .from("site-media")
          .getPublicUrl(brochure.metadata.file_path);

        if (data?.publicUrl) {
          window.open(data.publicUrl, "_blank", "noopener,noreferrer");
        } else {
          toast.error("Brochure file not found. Please contact support.");
        }
      } else {
        toast.error("Brochure file not found. Please contact support.");
      }
